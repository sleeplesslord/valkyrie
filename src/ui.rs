...
fn render_footer(f: &mut Frame, app: &App, area: Rect) {
    let help_text = match app.mode {
        Mode::Normal => {
            " j/k:nav | Enter:jump | w:worktree | r:rename | d:diff | D:diff window | x:cleanup | X:cleanup all | ?:help | q:quit "
        }
...
fn render_help_overlay(f: &mut Frame) {
    ...
    let help_text = vec![
        ...
        Line::from(" x       Cleanup offline agent"),
        Line::from(" X       Cleanup all offline agents"),
